...
public class VectorC implements InnerProductSpace<Complex>, VectorSpace<Complex> {
  ...
  @Override
  //Method to compute the Euclidean inner product
  public Complex inner(final VectorSpace<Complex> other){
    assert other instanceof VectorC;
    VectorC x = (VectorC)other;
    Complex sum = new Complex();
    if (this.size == x.size){
      for (int i=0; i<elem.length; i++){
        sum  = sum.add(this.elem[i].mul(x.elem[i]));
      }
    }
    return sum;
  }
  ...
  @Override
  public VectorC add(final VectorSpace<Complex> other){
    assert other instanceof VectorC;
    VectorC v = (VectorC)other;
    VectorC temp = new VectorC(this.size);
    if (this.size == v.size){
      for (int i=0; i<elem.length; i++){
          temp.elem[i] = this.elem[i].add(v.elem[i]);
      }
    } else {
  ...
  @Override
  public VectorC sub(final VectorSpace<Complex> other){
    assert other instanceof VectorC;
    VectorC v = (VectorC)other;
    VectorC temp = new VectorC(this.size);
    if (this.size == v.size){
      for (int i=0; i<elem.length; i++){
          temp.elem[i] = this.elem[i].sub(v.elem[i]);
