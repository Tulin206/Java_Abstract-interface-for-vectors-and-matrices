...
class MatrixD {
  ...
  public MatrixD mul(Real scalar){
    for (int i=0; i<elem.length; i++){
      for (int j=0; j<elem[i].length; j++){
        this.elem[i][j] = this.elem [i][j].mul(scalar);
      }
    }
    return this;
  }
  ...
  public MatrixD symm(){
    return (this.add(this.transpose())).mul(new Real(0.5));
  }


  public MatrixD antiSymm(){
    return (this.sub(this.transpose())).mul(new Real(0.5));
  }
  ...
  public boolean isAntiSymm(){
    return this.equals(this.transpose().mul(new Real(-1)));
  }
  ...
  public VectorD mul(VectorD vector){
    ...
    if (this.cols == vector.getSize()){
      Real[] result = new Real[elem.length];
      for (int i=0; i<elem.length; i++){
        Real sum = new Real();
        for (int j=0; j<elem[i].length; j++){
  ...
  //   assert x instanceof CoordinateVector;
  //   CoordinateVector vector = (CoordinateVector)x;
  //   VectorD resultVector = new VectorD(elem.length);
  //   if (this.cols == vector.getSize()){
  ...
  public static void test(MatrixD A, MatrixD B) {
    ...
    System.out.println("A+A==A.mul(2.0): " + A.mul(new Real(2.0)).equals(A.add(A)));
    System.out.println("B+B==B.mul(2.0): " + B.mul(new Real(2.0)).equals(B.add(B)));
